...
[Skip to content](https://docs.llamaindex.ai/en/stable/examples/retrievers/multi_doc_together_hybrid/#chunk-document-hybrid-retrieval-with-long-context-embeddings-togetherai)
...
*   [Examples](https://docs.llamaindex.ai/en/stable/examples/)
    ...
    *    Retrievers
        ...
        *    Chunk + Document Hybrid Retrieval with Long-Context Embeddings (Together.ai) [Chunk + Document Hybrid Retrieval with Long-Context Embeddings (Together.ai)](https://docs.llamaindex.ai/en/stable/examples/retrievers/multi_doc_together_hybrid/)
...
[![Image 3: Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/jerryjliu/llama_index/blob/main/docs/docs/examples/embeddings/together.ipynb)

Chunk + Document Hybrid Retrieval with Long-Context Embeddings (Together.ai)[¶](https://docs.llamaindex.ai/en/stable/examples/retrievers/multi_doc_together_hybrid/#chunk-document-hybrid-retrieval-with-long-context-embeddings-togetherai)
============================================================================================================================================================================================================================================
...
for idx, f in enumerate(all\_html\_files):
    ...
    \# Adjust this start\_idx to suit your needs
    start\_idx \= 64
    loaded\_doc \= Document(
        id\_\=str(f),
        text\="\\n\\n".join(\[d.get\_content() for d in loaded\_docs\[start\_idx:\]\]),
        metadata\={"path": str(f)},
    )
    print(str(f))
    docs.append(loaded\_doc)

reader = UnstructuredReader() # all\_files\_gen = Path("./docs.llamaindex.ai/").rglob("\*") # all\_files = \[f.resolve() for f in all\_files\_gen\] # all\_html\_files = \[f for f in all\_files if f.suffix.lower() == ".html"\] # curate a subset all\_html\_files = \[ "docs.llamaindex.ai/en/latest/index.html", "docs.llamaindex.ai/en/latest/contributing/contributing.html", "docs.llamaindex.ai/en/latest/understanding/understanding.html", "docs.llamaindex.ai/en/latest/understanding/using\_llms/using\_llms.html", "docs.llamaindex.ai/en/latest/understanding/using\_llms/privacy.html", "docs.llamaindex.ai/en/latest/understanding/loading/llamahub.html", "docs.llamaindex.ai/en/latest/optimizing/production\_rag.html", "docs.llamaindex.ai/en/latest/module\_guides/models/llms.html", \] # TODO: set to higher value if you want more docs doc\_limit = 10 docs = \[\] for idx, f in enumerate(all\_html\_files): if idx > doc\_limit: break print(f"Idx {idx}/{len(all\_html\_files)}") loaded\_docs = reader.load\_data(file=f, split\_documents=True) # Hardcoded Index. Everything before this is ToC for all pages # Adjust this start\_idx to suit your needs start\_idx = 64 loaded\_doc = Document( id\_=str(f), text="\\n\\n".join(\[d.get\_content() for d in loaded\_docs\[start\_idx:\]\]), metadata={"path": str(f)}, ) print(str(f)) docs.append(loaded\_doc)
