...
*   [Examples](https://docs.llamaindex.ai/en/stable/examples/)
    ...
    *    Agents
        ...
        *    Context-Augmented OpenAI Agent [Context-Augmented OpenAI Agent](https://docs.llamaindex.ai/en/stable/examples/agent/openai_agent_context_retrieval/)
            ...
            *   [Initial Setup](https://docs.llamaindex.ai/en/stable/examples/agent/openai_agent_context_retrieval/#initial-setup)
                ...
                *   [Use Uber 10-Q as context, use Calculator as Tool](https://docs.llamaindex.ai/en/stable/examples/agent/openai_agent_context_retrieval/#use-uber-10-q-as-context-use-calculator-as-tool)
...
[![Image 3: Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/run-llama/llama_index/blob/main/docs/docs/examples/agent/openai_agent_context_retrieval.ipynb)

Context-Augmented OpenAI Agent[¶](https://docs.llamaindex.ai/en/stable/examples/agent/openai_agent_context_retrieval/#context-augmented-openai-agent)
=====================================================================================================================================================
...
context\_agent.chat("What is the X and Z in September 2022?")
...
def magic\_formula(revenue: int, cost: int) \-> int:
    """Runs MAGIC\_FORMULA on revenue and cost."""
    return revenue \- cost

magic\_tool \= FunctionTool.from\_defaults(fn\=magic\_formula, name\="magic\_formula")

from llama\_index.core.tools import BaseTool, FunctionTool def magic\_formula(revenue: int, cost: int) -> int: """Runs MAGIC\_FORMULA on revenue and cost.""" return revenue - cost magic\_tool = FunctionTool.from\_defaults(fn=magic\_formula, name="magic\_formula")
...
context\_agent \= ContextRetrieverOpenAIAgent.from\_tools\_and\_retriever(
    \[magic\_tool\], sept\_index.as\_retriever(similarity\_top\_k\=3), verbose\=True
)

context\_agent = ContextRetrieverOpenAIAgent.from\_tools\_and\_retriever( \[magic\_tool\], sept\_index.as\_retriever(similarity\_top\_k=3), verbose=True )

In \[ \]:

Copied!

response \= context\_agent.chat(
    "Can you run MAGIC\_FORMULA on Uber's revenue and cost?"
)

response = context\_agent.chat( "Can you run MAGIC\_FORMULA on Uber's revenue and cost?" )
...
---------------------
Given the context information and not prior knowledge, either pick the corresponding tool or answer the function: Can you run MAGIC\_FORMULA on Uber's revenue and cost?

\=== Calling Function ===
Calling function: magic\_formula with args: {
  "revenue": 23270,
  "cost": 24960
}
Got output: -1690
