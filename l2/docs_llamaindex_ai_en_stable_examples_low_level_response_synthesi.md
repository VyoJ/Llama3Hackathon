...
[Skip to content](https://docs.llamaindex.ai/en/stable/examples/low_level/response_synthesis/#building-response-synthesis-from-scratch)
...
*   [Examples](https://docs.llamaindex.ai/en/stable/examples/)
    ...
    *    Low Level
        ...
        *    Building Response Synthesis from Scratch [Building Response Synthesis from Scratch](https://docs.llamaindex.ai/en/stable/examples/low_level/response_synthesis/)
...
[![Image 3: Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/jerryjliu/llama_index/blob/main/docs/docs/examples/low_level/response_synthesis.ipynb)

Building Response Synthesis from Scratch[¶](https://docs.llamaindex.ai/en/stable/examples/low_level/response_synthesis/#building-response-synthesis-from-scratch)
=================================================================================================================================================================
...
def combine\_results(
    ...
    if len(new\_texts) \== 1:
        ...
        return combine\_results(
            new\_texts, query\_str, qa\_prompt, llm, num\_children\=num\_children
        )

def generate\_response\_hs(
    retrieved\_nodes, query\_str, qa\_prompt, llm, num\_children\=10
):
    """Generate a response using hierarchical summarization strategy.

    Combine num\_children nodes hierarchically until we get one root node.
    ...
    for node in retrieved\_nodes:
        ...
        node\_response \= llm.complete(fmt\_qa\_prompt)
        node\_responses.append(node\_response)
        fmt\_prompts.append(fmt\_qa\_prompt)

    response\_txt \= combine\_results(
        \[str(r) for r in node\_responses\],
        query\_str,
        qa\_prompt,
        llm,
        fmt\_prompts,
        num\_children\=num\_children,
    )

    return response\_txt, fmt\_prompts
...
async def acombine\_results(
    ...
    if len(new\_texts) \== 1:
        ...
        return await acombine\_results(
            new\_texts, query\_str, qa\_prompt, llm, num\_children\=num\_children
        )

async def agenerate\_response\_hs(
    retrieved\_nodes, query\_str, qa\_prompt, llm, num\_children\=10
):
    """Generate a response using hierarchical summarization strategy.
    ...
    tasks \= \[llm.acomplete(p) for p in fmt\_prompts\]
    node\_responses \= await asyncio.gather(\*tasks)
