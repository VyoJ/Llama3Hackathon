...
[Skip to content](https://docs.llamaindex.ai/en/stable/examples/retrievers/ensemble_retrieval/#ensemble-retrieval-guide)
...
*   [Examples](https://docs.llamaindex.ai/en/stable/examples/)
    ...
    *    Retrievers
        ...
        *    Ensemble Retrieval Guide [Ensemble Retrieval Guide](https://docs.llamaindex.ai/en/stable/examples/retrievers/ensemble_retrieval/)
            ...
            *   [Define Ensemble Retriever](https://docs.llamaindex.ai/en/stable/examples/retrievers/ensemble_retrieval/#define-ensemble-retriever)
                
                *   [Analyzing the Relative Importance of each Chunk](https://docs.llamaindex.ai/en/stable/examples/retrievers/ensemble_retrieval/#analyzing-the-relative-importance-of-each-chunk)
...
[![Image 3: Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/run-llama/llama_index/blob/main/docs/docs/examples/retrievers/ensemble_retrieval.ipynb)

Ensemble Retrieval Guide[¶](https://docs.llamaindex.ai/en/stable/examples/retrievers/ensemble_retrieval/#ensemble-retrieval-guide)
==================================================================================================================================
...
from pathlib import Path from llama\_index.core import Document from llama\_index.readers.file import PyMuPDFReader
...
for chunk\_size in chunk\_sizes:
    ...
    splitter \= SentenceSplitter(chunk\_size\=chunk\_size)
...
\# initialize modules llm = OpenAI(model="gpt-4") chunk\_sizes = \[128, 256, 512, 1024\] nodes\_list = \[\] vector\_indices = \[\] for chunk\_size in chunk\_sizes: print(f"Chunk Size: {chunk\_size}") splitter = SentenceSplitter(chunk\_size=chunk\_size) nodes = splitter.get\_nodes\_from\_documents(docs) # add chunk size to nodes to track later for node in nodes: node.metadata\["chunk\_size"\] = chunk\_size node.excluded\_embed\_metadata\_keys = \["chunk\_size"\] node.excluded\_llm\_metadata\_keys = \["chunk\_size"\] nodes\_list.append(nodes) # build vector index vector\_index = VectorStoreIndex(nodes) vector\_indices.append(vector\_index)
...
from llama\_index.core.selectors import PydanticMultiSelector
...
from llama\_index.core.selectors import PydanticMultiSelector from llama\_index.core.retrievers import RouterRetriever from llama\_index.core.retrievers import RecursiveRetriever from llama\_index.core import SummaryIndex # the derived retriever will just retrieve all nodes summary\_index = SummaryIndex(retriever\_nodes) retriever = RecursiveRetriever( root\_id="root", retriever\_dict={"root": summary\_index.as\_retriever(), \*\*retriever\_dict}, )
...
\# define reranker
from llama\_index.core.postprocessor import LLMRerank, SentenceTransformerRerank
...
\# define reranker from llama\_index.core.postprocessor import LLMRerank, SentenceTransformerRerank from llama\_index.postprocessor.cohere\_rerank import CohereRerank # reranker = LLMRerank() # reranker = SentenceTransformerRerank(top\_n=10) reranker = CohereRerank(top\_n=10)
...
\# define RetrieverQueryEngine from llama\_index.core.query\_engine import RetrieverQueryEngine query\_engine = RetrieverQueryEngine(retriever, node\_postprocessors=\[reranker\])
